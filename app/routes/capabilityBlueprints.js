...
function implementationMessages({
  ...
  const messages = [{
    ...
    content: [
      ...
      "A fixed ContextDB effect already owns its currentValue and newValue transition. Do not add either state again as an ordinary input; the effect dependency proves the precondition locally and the declared output reports the result.",
      "A constant command verb such as wash selects the operation; it is not an ordinary action input. In a declared one-variable invocation family, only the varying referent is an input unless the contract explicitly declares another parameter.",
