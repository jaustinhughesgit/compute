...
router.get('/', async function(req, res, next) {
    ...
    signer.getSignedUrl({ url: url, policy: policy }, (err, signedUrl) => {
        if (err) {
            console.error('Error generating signed URL', err);
            return res.status(500).json({ message: 'Internal Server Error' });
        }

        // Set the signed URL as a cookie
        res.cookie('SignedUrl', signedUrl, { maxAge: twoMinutes, httpOnly: true });
        res.render('home', { title: 'Test', signedUrl: signedUrl });
    });
});
