...
function setupRouter(privateKey, dynamodb, dynamodbLL, uuidv4, s3, ses, openai, Anthropic) {
  ...
  _shared.use(async (ctx) => {
    ...
    // ───────────────────────────────────────────────────────────────────
    // First-time cookie → auto-provision group, user, and attach uid
    // ───────────────────────────────────────────────────────────────────
    try {
      // 'existing' is only set on newly minted cookies in manageCookie's return.
      if (ck?.existing && !ck.uid) {
        // 1) Create a fresh group + head entity so we can get entityID
        const groupName = `user-${ck.gi}`;     // arbitrary but stable
        const headName  = `profile`;          // short, readable head entity
        const ngCtx = {
          ...ctx,
          // newGroup expects "/<name>/<head>/<uuid?>"
          path: `/${groupName}/${headName}`,
          // ensure a body object exists (module reads ctx.req.body in places)
          req: { ...ctx.req, body: ctx.req.body || {} },
        };
        const ngRes = await _shared.dispatch("newGroup", ngCtx, { cookie: ck });
        const entityID = ngRes?.response?.created?.entityID;

        if (!entityID) {
          console.warn("newGroup did not return entityID; skipping user creation.");
          return;
        }

        // 2) Compose the pseudo-email from entityID
        const email = `${entityID}@email.1var.com`;

        // 3) Create the user (auto-IDs if userID not provided)
        const userCtx = {
          ...ctx,
          req: {
            ...ctx.req,
            body: {
              email,           // createUser will hash this into emailHash
              pubEnc: "",      // can be filled later
              pubSig: "",
              revoked: false,
            },
          },
        };
        const createUserRes = await _shared.dispatch("createUser", userCtx);
        const userID = createUserRes?.userID;

        if (!userID) {
          console.warn("createUser did not return userID; skip attaching to cookie.");
          return;
        }

        // 4) Attach uid to the cookie row
        await _shared
          .getDocClient()
          .update({
            TableName: "cookies",
        ...
        // Also mirror on the request so downstream handlers see it this request
        ctx.req.cookies.uid = String(userID);
      }
    } catch (e) {
      console.error("auto-provision (cookie→group→user) failed:", e);
      // Intentionally non-fatal: cookie still works, user can be created later.
    }
