...
New Position postings use `AB2` partition-key shards and carry canonical revision/content evidence. Compute manifest registration writes an exact contract hash plus a sanitized semantic Position, with tenant postings for the owner and global postings only for public definitions. Search reads v2 plus legacy v1 candidates through a bounded, configurable band window (160 by default) and makes one configurable 320-band pass only when every initial source is empty; it never substitutes a registry scan. It reloads current subdomain records, derives visibility from server state, distinguishes read visibility from an explicit `use` grant, authorizes the caller, and only then ranks. Stale-posting deletion, existing-definition backfill, exact reranking, and deployed scale/cost gates remain work. Retrieval projections cannot supply identity, facts, or authorization.
