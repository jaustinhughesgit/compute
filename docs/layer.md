...
Compute discovery also emits intent-jurisdiction v1. Existing exact contracts produce reuse, failures inside an existing operation produce repair, an added operation produces fork, and no compatible contract produces build. The legacy `extend` field remains during client migration, but cannot erase the typed evolution outcome. This is a decision foundation; immutable compatible releases, persisted fork lineage, user installations, promotion, and dependency-range enforcement remain incomplete.
