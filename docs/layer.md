...
The v1 Convert authoring envelope may supply ordered `requirementSegments`. Compute validates their count and size, derives one combined request, and exposes the boundaries to capability discovery. This authoring contract carries no implicit Essence or ContextDB evidence; a future contextual attachment requires an explicit typed reference. See [decision 0038](../../architecture/decisions/0038-convert-hard-stops-compose-requirements.md).
