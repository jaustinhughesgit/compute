...
This is a phase-7–9 foundation, not a phase-10 transport claim. `runEntity` now applies canonical `execute` governance to its current target, but is not permitted to start executing additional ancestors merely because the middleware library exists. Browser-main, `fileWorker`, and Compute/JPL adoption must preserve their separate trust and effect boundaries.
