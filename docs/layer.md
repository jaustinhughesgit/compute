...
The v1 Convert authoring envelope may supply ordered `requirementSegments`. Compute validates their count and size, derives one combined request, and exposes the boundaries to capability discovery. When a bounded requirement explicitly declares what the user will ask, say, type, enter, or request, Compute deterministically retains that clause as an utterance example on the selected operation instead of allowing a discovery paraphrase to replace it. Discovery is semantic-first: a build response declares a structured answer plan before its capability request, and Compute reconciles the operation/input/output plus any ContextDB subject/property address against that plan before executable generation. An explicit ContextDB subject/property binding is part of the reusable contract, not attached Context data; generated current-user aliases normalize to canonical `speaker`, while no current value is read or copied during Convert. The deictic owner is binding-address metadata rather than an ordinary value. Generated discovery may repair an undeclared owner-as-utterance-value into the planned ContextDB property input, canonicalizes an owner-named ContextDB input to its non-deictic property name or merges an identical property binding, strips a redundant deictic prefix fused into that speaker-owned property when the remaining identifier exactly matches its generated input, and EntityPlan augmentation cannot re-add a redundant current-speaker pseudo-input. An explicitly declared property, explicitly declared input name, or non-deictic person example remains untouched. Background discovery gets one validation-informed semantic correction; a second invalid result fails closed. Every normalized operation carries a nonempty answer template so Compute acceptance and browser Path validation agree. Browser compilation and acknowledgement remain the Path-signature authority. This authoring contract carries no implicit Essence or ContextDB value evidence; a concrete contextual attachment requires an explicit typed invocation reference. See [decisions 0038](../../architecture/decisions/0038-convert-hard-stops-compose-requirements.md) and [0044](../../architecture/decisions/0044-semantic-answer-plans-precede-compute-contracts.md).
