...
New Position postings use `AB2` partition-key shards and carry canonical revision/content evidence. Search reads v2 plus legacy v1 candidates, reloads current subdomain records, derives visibility from server state, authorizes the caller, and only then ranks. Stale-posting deletion, exact reranking, and deployed scale/cost gates remain work. Retrieval projections cannot supply identity, facts, or authorization.
...
Compute discovery also emits intent-jurisdiction v1. Existing exact contracts produce reuse, failures inside an existing operation produce repair, an added operation produces fork, and no compatible contract produces build. A post-local-proof request carries only bounded failure interpretation as untrusted jurisdiction evidence, so an external request is not forced back into a local query contract. Cross-owner registry candidates require an active canonical `use` grant and execution authorizes again; local Paths, ordinary bindings, and Protected Assets are not shared with the definition. The legacy `extend` field remains during client migration, but cannot erase the typed evolution outcome. This is a decision foundation; immutable compatible releases, persisted fork lineage, first-class user installations, indexed candidate retrieval, promotion, and dependency-range enforcement remain incomplete.
