...
Compute capability reuse does not treat a model-generated `capabilityIdHint` as a universal signature. Registry writes derive an exact SHA-256 contract fingerprint plus a sanitized semantic document. Position indexes the semantic document, capability discovery uses a bounded 512-band candidate window and top-K cap, and the registry reloads current manifests and explicit `use` grants before discovery may select reuse. The wider capability-specific window prevents short paraphrases from falling outside the narrower interactive Search default without allowing a table scan. Public visibility permits discovery/read only; Convert-created reusable public compute definitions carry a separate canonical `pub` use grant. Reuse returns the exact manifest; it never loads the creator's Path or local bindings. Existing definitions require backfill before this path can find them at catalog scale. See [decision 0039](../../architecture/decisions/0039-compute-contract-fingerprints-and-positioning.md) and [decision 0048](../../architecture/decisions/0048-compute-installations-bind-entity-use-by-exact-id.md).
