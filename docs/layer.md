...
Bulk Path persistence validates every submitted Path before writing any member of the batch. A validation failure returns the rejected signatures with zero writes; callers must never interpret a partially accepted foundation dataset as complete. Its structural schema accepts browser-owned Pattern Schema v3 and v4 envelopes plus the vocabulary-neutral `entity_list` slot used to persist repeated typed roles. For v4, Compute validates the signature namespace, bounded ordered subpattern references, complete-coverage declaration, slots, transform, tests, and quality evidence; it does not activate subpatterns or interpret the browser-local composition.
