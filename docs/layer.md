...
Compute now exposes one canonical persistence port at `app/persistence/canonicalPersistence.js`. It isolates physical Words, entities, subdomains, groups, links, versions, legacy access/verification, grants, identity support, the Context compatibility sidecar, and Position/Search postings. Shared entity getters and active Context-sidecar access use this port while keeping their existing response envelopes. New canonical-family modules should use the port rather than importing table names or compact legacy attributes.

The port is an implementation foundation, not a cutover claim. Context records are still stored in the sidecar until publication compilation and backfill phases; several creation, relationship, capability, and retrieval modules still contain direct physical access and are tracked in the architecture compatibility inventory. Position/RAG postings are explicitly derived candidates and cannot supply identity, facts, or authorization.
