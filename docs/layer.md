...
The active Context graph v1 actions verify that the path-selected workspace belongs to the authenticated principal. The current speaker maps to a stable principal node; other people resolve only by exact unique public handles. Compute derives connected-component audiences from those resolved identities, stores one versioned node/relation view per participant audience, and returns stable IDs in the acknowledgement. Hydration queries only `u:<authenticated principal>`, never a client-selected audience. Removed relations are tombstoned for current and former audiences, and records retain publisher/source provenance. The retained encrypted `ContextGraphTable` is separate from legacy creator exports and protected-asset storage. See [decision 0021](../../architecture/decisions/0021-participant-scoped-context-publication.md).
