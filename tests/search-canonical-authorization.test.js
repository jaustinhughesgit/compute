...
test("Search expands one bounded band when the initial semantic window is empty", async (t) => {
  const originalLoad = anchors.loadAnchors;
  const originalAssign = anchors.assign;
  t.after(() => {
    anchors.loadAnchors = originalLoad;
  ...
  const windows = [];
  const persistence = {
    retrieval: { queryWindow: async ({ partitionKey, endKey }) => {
      windows.push(endKey);
      const expanded = String(endKey).includes("00420");
      return {
        Items: expanded && partitionKey === "AB2#anchors_v1#L0=1#L1=2#S=00"
          ? [{ pk: partitionKey, sk: "B=00300#T=capability#SU=entity-report", su: "entity-report", band: 300 }]
          : [],
      };
    } },
    foundation: { addresses: { batchGet: async () => [{ su: "entity-report", e: "10", z: true }] } },
    authorization: { batchGetGrants: async () => [{
      entityID: "entity-report",
      principalID: "pub",
  ...
  const result = await handlers.get("search")({
    req: { body: { embedding: [1, 0], topK: 5 } },
    res: {},
  }, { cookie: { e: "12" } });
  assert.equal(result.ok, true);
  assert.equal(result.response.params.bandWindow, 160);
  assert.equal(result.response.params.effectiveBandWindow, 320);
  assert.deepEqual(result.response.results.map((row) => row.su), ["entity-report"]);
  assert.ok(windows.some((endKey) => String(endKey).includes("00260")));
  assert.ok(windows.some((endKey) => String(endKey).includes("00420")));
});
